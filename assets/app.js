...
var header = {
  init: function(){
    ...
    $('.mobile_menu span').on('click', function(e) {
      var $toggle = $(this);
    ...
    $('.mobile_menu .text_link').on('click', function(e) {
...
var slideshow = {
  init: function(){
    $('.homepage_slider').each(function (index, value){
      ...
      var slideshowTextAnimation = homepageSlider.data('slideshow-text-animation')
      ...
      homepageSlider.flexslider({
        ...
        start: function(slider){
          if (slideshowTextAnimation) {
            $(slider).find(".flex-active-slide").find('.container').addClass("animated " + slideshowTextAnimation );
          }

        },
        before: function(slider){
          $(slider).find(".flex-active-slide").find('.container').each(function(){
            $(this).removeClass("animated " + slideshowTextAnimation);
          });
        },
...
var testimonial = {
  init: function(){
    $('.testimonial_slider').each(function (index, value){
      ...
      var slideshowTextAnimation = testimonialSlider.data('slideshow-text-animation')
...
$(function() {
  ...
  $('.update_subtotal').click(function() {
    $("#cart_form").submit(); return false;
  });

 const $cart_form = $("#cart_form");

  // Quantity change => submit form to get total price
  $("#cart_form .quantity").change(function(e) {
    e.preventDefault();
    e.stopPropagation();
    $.ajax({
      ...
      success: function (data) {
        console.log(data)
        const $total_money = $('.total-money .money');
      	// Update total money
        $total_money[0].innerHTML = `${new Intl.NumberFormat('en-US', {style: 'currency', currency: data.currency}).format((data.items_subtotal_price/100).toFixed(2))} ${data.currency}`;
      
        // Update total money for items in cart
        data.items.forEach((item) => {
          const $line_money = $(`#line_price_${item.id}`)[0].innerHTML = `${new Intl.NumberFormat('en-US', {style: 'currency', currency: data.currency}).format((item.line_price/100).toFixed(2))}`;
        })
      },
      error: function(data) {
        console.log(data)
  ...
  $('.qtyplus').click(function(e){
    ...
    const itemId = DOMId.substr(DOMId.indexOf('_')+1);
    const quantityElement = $(`#updates_${itemId}`);
    
    quantityElement[0].value = parseInt(quantityElement[0].value) + 1;
    // Notify when one of quantity inputs changes
        ...
        $.ajax({
          ...
          success: function (data) {
            $cf.fadeOut("slow", function(){
              $cf.prev('.message').html("One more step! Please check your email to confirm your subscription!");
            });
          },
          error: function(data) {
  ...
  function refreshCart(cart) {
    ...
    if ($cartBtn) {
      ...
      if(cart.item_count == 0) {
        ...
        $cartCount.html('<span class="cart_count"> (' + cart.item_count + ')</span>');
        $cartBtn.addClass('active');
      } else {
        $cartBtn.append('<span class="cart_count"> (' + cart.item_count + ')</span>');
        $cartBtn.addClass('active');
      }
    }
  }
  ...
  selectCallback = function(variant, selector) {
    ...
    if (variant && variant.featured_image && $product.is(":visible")) {
      var $sliders = $('.product_slider', $product);
    ...
    // Quick Shop
    if (variant && variant.featured_image && $product.is(":visible") && !$productForm.hasClass("init")) {
        ...
        success: function(data) {
          ...
          if(data.meta.code === 200 && data.data.length) {
            ...
            for(var i = 0; i < data.length; i++) {
              ...
              if(item) {
                item = '<div class="instas">'+item+'</div>';
              }
              if(item !== '') {
                s.el.append(item);
                count++;
...
$(document)
  .on('shopify:section:load', function(e){
    ...
    if ($parentSection.hasClass('twitter-feed-section')){
      $('.twitter-feed-wrap').each(function (index, value) {
        var twitterEnabled = $(this).data('twitter');
        social.twitter();
      });
    }
...
$(document)
	.on('click','.promo-list .promo-list__action--wrapper', function(event){
...
$(document)
	.on('click', '.sidebar_mobile_wrapper .sidebar_mobile', function(event){
...
$(document).on('click', function(event){
	...
	if ($('body').hasClass('open_sidebar')&&($target.closest('.page-sidebar').length === 0)&&($target.closest('.sidebar_mobile').length === 0)){
		$('body').removeClass('open_sidebar');
		$('.page-sidebar').removeClass('is-open');
		$('.sidebar_mobile_wrapper').removeClass('is-open');
	}
})

gsap.fromTo("#img404", {width:0, opacity:0}, {duration: 3, width:500, opacity:1, scale: 0.5});